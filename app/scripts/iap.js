function updateProductsHTML(id, json, position) {
	...
	for (var i=0; i<json.length; i++) {
		...
		if (position === 'center') {
			htmlCode += '<a href="buyproduct://' + json[i].id + '"><div class="XLT LT MT ST PT"></div><div class="item-container XL12 XL-half L12 L-half M12 M-half S12 S-half P12 P-half item-container-app has-image"><div class="item-inner"><a class="image portrait-img" target="_blank"><figure class="loading" data-url="' + json[i].image + '"></figure></a><h2 class="item-headline"><div class=item-tag><a>FT电子书</a></div>' + json[i].headline + '</h2><div class="item-lead">' + json[i].lead + '</div><div class="item-bottom"></div></div></div></a>';
		} else {
